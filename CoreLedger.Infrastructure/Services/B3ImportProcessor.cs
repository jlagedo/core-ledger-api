...
public class B3ImportProcessor : IB3ImportProcessor
{
    ...
    public async Task ProcessAsync(int coreJobId, string referenceId, CancellationToken cancellationToken = default)
    {
        ...
        await strategy.ExecuteAsync(async () =>
        {
            ...
            _logger.LogInformation("Starting B3 import for CoreJob {CoreJobId}, ReferenceId {ReferenceId}", coreJobId, referenceId);
            _logger.LogInformation("========================================");
            ...
            while (offset < totalCount)
            {
                ...
                _logger.LogInformation("----------------------------------------");
                _logger.LogInformation("Batch {BatchNumber} | Progress: {Progress}% | Offset: {Offset}/{TotalCount}", 
                ...
                _logger.LogInformation("Loading existing securities for {TickerCount} unique tickers...", tickers.Count);
                ...
                _logger.LogInformation("Found {ExistingCount} existing securities in database", existingSecurities.Count);
                ...
                foreach (var instrument in instruments)
                {
                    try
                    {
                        ...
                        if (!existingSecurities.TryGetValue(tickerKey, out var existingSecurity))
                        {
                            ...
                            if (batchCreated <= 3)
                                _logger.LogDebug("Creating new security: {Ticker} - {Name}", instrument.Ticker, instrument.InstrumentName);
                        }
                        else
                        {
                            ...
                            if (batchUpdated <= 3)
                                _logger.LogDebug("Updating security: {Ticker} - {Name}", instrument.Ticker, instrument.InstrumentName);
                        }
                ...
                _logger.LogInformation("Batch {BatchNumber} summary: Created={Created}, Updated={Updated}, Failed={Failed}", 
            ...
            _logger.LogInformation("Success Rate: {Rate}%", totalCount > 0 ? (int)((double)processedCount / totalCount * 100) : 100);
            _logger.LogInformation("========================================");
            }
            catch (Exception ex)
            {
